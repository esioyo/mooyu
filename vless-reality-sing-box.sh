...
# 准备基础命令：安装 curl 和 wget，供后续下载安装脚本使用。
...
# 安装 sing-box 并生成 VLESS Reality 配置。
install_sing_box() {
    if [[ -z $port ]]; then
        read -p "按回车键开始安装 sing-box..."
    fi

    echo
    echo -e "$yellow准备工作$none"
    echo "----------------------------------------------------------------"
    apt update
    ...
    echo
    echo -e "$yellow配置 sing-box VLESS Reality$none"
    echo "----------------------------------------------------------------"
...
show_menu() {
    ...
    echo -e "        |_____|               |_____|        "
    echo
    echo -e "$cyan========== sing-box VLESS Reality 安装脚本 ==========$none"
    echo
